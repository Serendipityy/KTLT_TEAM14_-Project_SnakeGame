...
bool IsValidObstacle(int x, int y, int width, int height) {
	if (food[FOOD_INDEX].x >= x && food[FOOD_INDEX].x < x + width
		&& food[FOOD_INDEX].y >= y && food[FOOD_INDEX].y < y + height)
		return false;
	return true;
}
void GenerateFood() {
...
void Eat() {
	...
	if (FOOD_INDEX == MAX_SIZE_FOOD - 1)
	{
		...
		//Create gate 
		GenerateGate(3, 2);
		DrawGate(gate[GATE_INDEX].x, gate[GATE_INDEX].y, 3, 2);
	}
	else {
		Level(LEVEL);
		FOOD_INDEX++;
...
{
	if (x >= obstacle[OBSTACLE_INDEX - 1].x && x <= obstacle[OBSTACLE_INDEX - 1].x + width - 1
...
void MoveRight() {
	if (snake[SIZE_SNAKE - 1].x + 1 == WIDTH_CONSOLE + 8 || (SnakeTouchBody(snake[SIZE_SNAKE - 1].x + 1, snake[SIZE_SNAKE - 1].y) == true)
		...
		|| SnakeTouchGate(snake[SIZE_SNAKE - 1].x + 1, snake[SIZE_SNAKE - 1].y, 3, 2) == true) {
		ProcessDead();
	}
	else {
...
void MoveLeft() {
	if (snake[SIZE_SNAKE - 1].x - 1 == 8 || (SnakeTouchBody(snake[SIZE_SNAKE - 1].x - 1, snake[SIZE_SNAKE - 1].y) == true)
		|| SnakeTouchObstacle(snake[SIZE_SNAKE - 1].x - 1, snake[SIZE_SNAKE - 1].y, 4, 4) == true
		|| SnakeTouchGate(snake[SIZE_SNAKE - 1].x, snake[SIZE_SNAKE - 1].y, 3, 2) == true) {
		ProcessDead();
	}
	else {
		if (snake[SIZE_SNAKE - 1].x - 1 == food[FOOD_INDEX].x && snake[SIZE_SNAKE - 1].y == food[FOOD_INDEX].y) {
...
void MoveDown() {
	if (snake[SIZE_SNAKE - 1].y + 1 == HEIGH_CONSOLE || (SnakeTouchBody(snake[SIZE_SNAKE - 1].x, snake[SIZE_SNAKE - 1].y + 1) == true)
		...
		|| SnakeTouchGate(snake[SIZE_SNAKE - 1].x, snake[SIZE_SNAKE - 1].y + 1, 3, 2) == true) {
		ProcessDead();
	}
