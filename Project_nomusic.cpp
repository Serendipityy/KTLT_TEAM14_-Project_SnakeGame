...
void GenerateObstacle(int width, int height) {
	...
	for (int i = 0; i < MAX_SIZE_OBSTACLE; i++) {
		do {
			x = rand() % (WIDTH_CONSOLE - 1 - width) + 1;
...
void Eat() {
	...
	if (FOOD_INDEX == MAX_SIZE_FOOD - 1)
	{
		...
		if (LEVEL == 1) { //Tạo chướng ngại vật màn 1
			GenerateObstacle(4, 4);
			OBSTACLE_INDEX++;
			if (OBSTACLE_INDEX == MAX_SIZE_OBSTACLE - 1)
				OBSTACLE_INDEX == 0;
		}
		FOOD_INDEX++;
		SIZE_SNAKE++;
...
{
	if (x >= obstacle[OBSTACLE_INDEX].x && x <= obstacle[OBSTACLE_INDEX].x + width
		&& y >= obstacle[OBSTACLE_INDEX].y && y <= obstacle[OBSTACLE_INDEX].y + height)
		{
			return true;
		}
	return false;
